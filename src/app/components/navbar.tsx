...
"use client";
import Link from "next/link";
import { IoSearch, IoClose, IoHeartOutline, IoHeart } from "react-icons/io5";
import { MdOutlineShoppingCart } from "react-icons/md";
import { CgProfile } from "react-icons/cg";
  ...
  return (
    <header className="w-full h-auto flex flex-col items-center bg-white px-4 sm:px-6 lg:px-8 py-4 relative">
      <div className="w-full flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
          <motion.div variants={iconVariants} whileHover="hover" whileTap="tap" onClick={toggleSearchBar} className="cursor-pointer">
            <IoSearch className="text-xl" />
          </motion.div>
        </div>

        <div className="flex items-center gap-4">
          <motion.div className="relative cursor-pointer text-black" variants={iconVariants} whileHover="hover" whileTap="tap">
            <SignedOut>
              <SignInButton>
          ...
          <SignedIn>
            ...
            <SignOutButton>
              <button className="ml-2 bg-red-500 text-white px-3 py-1 rounded-lg hover:bg-red-600 transition-all">
                Sign Out
              </button>
            </SignOutButton>
          </SignedIn>
      ...
      <div className="hidden lg:flex w-full justify-center items-center py-4">
        <nav className="flex gap-6 justify-center w-full">
          {navLinks.map((link) => (
            <Link key={link.label} href={link.href} className="hover:text-[#5a526c] border-b-2 border-transparent hover:border-[#5a526c] pb-1">
              {link.label}
            </Link>
          ))}
        </nav>
